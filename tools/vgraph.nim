...
proc layerOfModule(modulePath: string, order: seq[string]): int =
  ...
  ## name against any path component, so `UniGlyph/glyph/outline` resolves to
...
proc main() =
  ...
  # Family DAG: sibling Uni* engines ride --path (config.nims), never
  # `nimble requires`. Any Uni* named in `requires` is a back-edge; the
  # [engines] section documents this repo's DAG position, it is not an
  # allowlist.
  var engines = 0
  if fileExists(Nimble):
    for package in requiredPackages(Nimble):
      ...
      inc engines
      violations.add &"{Nimble}: requires {package}; sibling engines ride --path, not requires"
  ...
  echo &"vgraph: {checked} modules respect {order.join(\" < \")}; " &
       &"{engines} sibling-engine requires (forbidden; ride --path)"
