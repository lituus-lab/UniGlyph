...
```text
feat(core): parse the glyf table
fix(c_api): clamp negative size instead of raising
docs: clarify layer order
feat(layout)!: drop the old advance API
```
...
2. Pass the gates: `nimble testAll`, `nimble ctest`, `nimble cexample`,
...
NimContracts compiled away under `-d:release`; the C ABI never raises — it maps
errors to `UGLY_*` codes. UniGlyph is an original implementation; do not
reproduce third-party source.
